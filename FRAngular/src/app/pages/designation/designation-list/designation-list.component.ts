...
export class DesignationListComponent implements AfterViewInit {
  length: any;
  displayedColumns: string[] = ['sl', 'nameBangla', 'nameEnglish', 'department', 'recStatus', 'actions'];
  searchString: string = '';
    ...
    merge(this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.designationService.getAllDesignationByPage(
            {
  ...
  addNew() {
    ...
    let dialogRef = this.dialog.open(DesignationAddComponent, dialogConfig)
  ...
  deleteRow(row: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "confirm") {
        this.asyncService.start();
        this.designationService.deleteDesignation(row.designationId).subscribe(
          (data) => {
            if (data) {
  ...
  restoreRow(row: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "confirm") {
        this.designationService.restoreDesignation(row.designationId).subscribe(
          (data) => {
            this.asyncService.finish();
            if (data) {
              if (data.isExecute === true) {
