...
@Component({
  selector: 'app-assign-role-list',
  templateUrl: './assign-role-list.component.html',
  styleUrls: ['./assign-role-list.component.scss']
})
export class AssignRoleListComponent implements AfterViewInit {
  length: any;
  displayedColumns: string[] = ['sl', 'userId', 'roleName', 'recStatus', 'actions'];
  searchString: string = '';
    ...
    merge(this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.assignRoleService.getAllAssignedRoleByPage(
            {
  ...
  addNew() {
    ...
    let dialogRef = this.dialog.open(AssignRoleAddComponent, dialogConfig)
  ...
  deleteRow(row: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == "confirm") {
        this.asyncService.start();
        this.assignRoleService.deleteAssignedRole(row.roleAssignId).subscribe(
          (data) => {
            if (data) {
